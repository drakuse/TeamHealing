...
<script type="text/javascript">
		...
		if (document.getElementById("MEMBER_EMAIL").value != ''
			...
			$
					.ajax({
						...
						url : "createKey",
						data : {
							"MEMBER_EMAIL" : memberModifyForm.MEMBER_EMAIL.value
						},
						dateType : "text",
	...
	function sendForm() {
		...
		if (pw == "") {
			...
			document.memberModifyForm.MEMBER_PASS.focus();
			return false;
		} else if (memberModifyForm.MEMBER_PASS.value == memberModifyForm.MEMBER_ID.value) {
			// 아이디와 비밀번호 같은지 확인 여부
			...
			document.memberModifyForm.MEMBER_PASS.focus();
			document.memberModifyForm.MEMBER_PASS.select();
			return false;
		} else if (pw != pwcon) {
			//비밀번호와 비밀번호 확인 일치여부 체크
			alert("비밀번호가 일치하지 않습니다")
			document.memberModifyForm.passwordConfirm.value = ""
			document.memberModifyForm.passwordConfirm.focus();
			return false;
		} else if (name == "") { // 이름 입력 여부 확인 
			alert("이름을 입력하지 않았습니다.")
			document.memberModifyForm.MEMBER_NAME.focus();
			return false;
		} else if (name < 2) { // 이름 글자수 확인
			alert("이름을 2자 이상 입력해주십시오.")
			document.memberModifyForm.MEMBER_NAME.focus();
			document.memberModifyForm.MEMBER_NAME.select();
			return false;
		} else if (phone == "") { // 전화번호 입력 여부 확인
			alert("전화번호를 입력해주세요.");
			document.memberModifyForm.MEMBER_PHONE.focus();
			return false;
		} else if (phone.length > 14 || phone.length < 0) {
			// 전화번호 형식 확인
			alert("전화번호 형식을 맞춰주세요.");
			document.memberModifyForm.MEMBER_PHONE.focus();
			document.memberModifyForm.MEMBER_PHONE.select();
			return false;
		} else if (email == "") {
			// 이메일 입력 확인
			alert("이메일을 입력하지 않았습니다.")
			document.memberModifyForm.MEMBER_EMAIL.focus();
			return false;
		} else if (emailReg.test(email) === false) {
			// 이메일 형식
			alert("잘못된 이메일 형식입니다.");
			document.memberModifyForm.MEMBER_EMAIL.value = ""
			document.memberModifyForm.MEMBER_EMAIL.focus();
			return false;
			...
			alert("상세주소를 기입해주세요.");
			document.memberModifyForm.MEMBER_DETAILADDRESS.focus();
			return false;
		} else if (division == 2) {
			...
			if (business == "") {
				alert("사업자번호를 입력해주세요.");
				document.memberModifyForm.MEMBER_BUSINESS.focus();
				return false;
			} else if (company == "") {
				alert("회사명을 입력해주세요.");
				document.memberModifyForm.MEMBER_COMPANY.focus();
				return false;
			}
		}
		...
		if (document.getElementById("mailTestIn").value != '') {
			...
			$.ajax({
				...
				url : 'checkKey',
				data : {
					"key" : memberModifyForm.mailTestIn.value
				},
				dataType : 'text',
				success : function(data) {
					if (data == "1") {
...
<title>memberModifyForm.jsp 파일입니다.</title>
</head>
<body>
	<!-- start header -->
	<header>
	...
	<div
		...
		<form action="memberModify" name="memberModifyForm"
			id="memberModifyForm" method="post">
				...
				<c:when test="${MEMBER_DIVISION == 1}">
					...
					<div class="form-group">
						<label for="MEMBER_NAME">이름</label> <input type="text"
							...
							value=${memberModify.MEMBER_NAME }>
					</div>

					<div class="form-group">
						<label for="MEMBER_AGE">나이</label> <input type="email"
							class="form-control" id="MEMBER_AGE" name="MEMBER_AGE"
							placeholder="나이" required="required"
							value="${memberModify.MEMBER_AGE }">
					</div>
					...
					<div class="form-group">
						<label for="MEMBER_PHONE">전화번호</label> <input type="text"
							...
							value="${memberModify.MEMBER_PHONE }">
					</div>
					...
					<div class="form-group" id="passForm">
						<label for="MEMBER_EMAIL">이메일 <input
							...
							value="${memberModify.MEMBER_EMAIL }">
					</div>
					...
					<div class="form-group">
						<label>주소 <input type="button"
							...
							placeholder="우편번호" value="${memberModify.MEMBER_POSTCODE }">
						<br> <input type="text" class="form-control"
							id="MEMBER_JIBUNADDRESS" name="MEMBER_JIBUNADDRESS"
							placeholder="지번주소" value="${memberModify.MEMBER_JIBUNADDRESS }"
							required="required"> <span id="guide"
							style="color: #999; display: none"></span> <input type="text"
							class="form-control" id="MEMBER_DETAILADDRESS"
							name="MEMBER_DETAILADDRESS" placeholder="상세주소"
							value="${memberModify.MEMBER_DETAILADDRESS }" required="required">
					</div>
				...
				<c:otherwise>
					...
					<div class="form-group">
						<label for="MEMBER_BUSINESS">사업자 번호</label> <input type="text"
							...
							value="${memberModify.MEMBER_BUSINESS }">
					</div>
					...
					<div class="form-group">
						<label for="MEMBER_COMPANY">업소명</label> <input type="text"
							...
							placeholder="사업자 번호" required="required"
							value="${memberModify.MEMBER_COMPANY }">
					</div>
				</c:otherwise>
			</c:choose>
