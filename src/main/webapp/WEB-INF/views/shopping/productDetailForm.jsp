				...
				<div class="col-lg-12">
					<div class="big-cta">
						<div class="cta-text">
							...
							<h4>수량</h4>
							<input type="button" onclick="change(-1)" value="-"> <input
								type="text" name="amount" id="amount" value="1"> <input
								type="button" onclick="change(1)" value="+"> <br> <br>
		...
		<div style="width: 100px; margin: auto;">
			<c:choose>
				...
				<c:when test="${pageVO.page>1 }">
					<a
						href="productDetailForm?PRODUCT_NUM=${productDetail.PRODUCT_NUM }&page=${page }&page2=${pageVO.page-1}">[이전]</a>&nbsp;
   </c:when>
			</c:choose>
			...
			<c:forEach begin="${pageVO.startPage }" end="${pageVO.endPage }"
				var="i">
				<a
					href="productDetailForm?PRODUCT_NUM=${productDetail.PRODUCT_NUM }&page=${page }&page2=${i}">${i }</a>
			</c:forEach>
			<c:choose>
				<c:when test="${pageVO.page>=pageVO.maxPage }">[다음]&nbsp;</c:when>
				<c:when test="${pageVO.page<pageVO.maxPage }">
					<a
						href="productDetailForm?PRODUCT_NUM=${productDetail.PRODUCT_NUM }&page=${page }&page2=${pageVO.page+1}">[다음]</a>&nbsp;
   </c:when>
			</c:choose>
		</div>
	</c:if>
